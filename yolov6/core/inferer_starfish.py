...
class Inferer:
    ...
    def inferv2(self, big_image, conf_thres, iou_thres, classes, agnostic_nms, max_det, conf_threshold, overlap_threshold):
        size = 640
        ...
        for ind, (x, y) in enumerate(coordinates):
            ...
            img, img_src = self.process_image(image, 640, self.stride, self.half)
            ...
            for *xyxy, conf, cls in reversed(det):
                ...
                if block_mean > overlap_threshold:
                    continue
                if (x2-x1) * (y2-y1) < H*W//5184:
                    continue
                overlap_checker[y1:y2, x1:x2] = 255
                new_xyxy = [x1, y1, x2, y2]
                line = [int(cls), new_xyxy, int(conf)] # cls will always be 1, meaining brittle stars because other objects were filtered out
